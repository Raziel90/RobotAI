...
def doit(initial_pos, move1, move2, Z0, Z1, Z2):
    path = [initial_pos, move1, move2]
    landmarks = [Z0, Z1, Z2]
    landmark_confidence = [1, 1, 5]  # hardcoded for exercise
    actions = [[1, -1], [-1, 1]]
    n = len(path)
    m = len(landmarks)
    omega = [[0] * (n + 1) for i in range(n + 1)]
    xi = [0] * (n + 1)

    omega[0][0] += 1  # x0 = initial_pos constraint
    xi[0] += path[0]
    for i in range(1, n):
        dx = -(path[i])
        omega[i - 1][i - 1] += actions[0][0]
        omega[i - 1][i] += actions[0][1]
        xi[i - 1] += dx
        omega[i][i - 1] += actions[1][0]
        omega[i][i] += actions[1][1]
        xi[i] += -dx
    ...
    omega = matrix(omega)
    xi = matrix([xi]).transpose()

    # omega = omega.expand(5,5,[0,1,2],[0,1])
    # omega = omega.value
    # print(omega)
    # print(xi)
    mu = omega.inverse() * xi
