...
def estimate_next_pos(measurement, OTHER=None):
    ...
    if OTHER is not None and len(OTHER['meas']) >= 2:
        ...
        angles = []
        current = measurement
        step = distance_between(OTHER['meas'][-1], measurement)
        for i in range(2):
            prev = OTHER['meas'][-1 - i]
            movement = [current[0] - prev[0], current[1] - prev[1]]
            angles.append(atan2(movement[1], movement[0]))
            current = prev
        OTHER['meas'] += [measurement]

        turning = angles[0] - angles[1]

        myrob = robot(OTHER['meas'][-1][0], OTHER['meas'][-1][1], angles[0])
        myrob.move(turning=turning, distance=step)
        xy_estimate = myrob.sense()
