...
class robot:
    ...
    def measurement_prob(self, measurements):
        ...
        predicted_measurements = self.sense(False)  # Our sense function took 0 as an argument to switch off noise.
    ...
    def sense(self, noisy=True):  # do not change the name of this function
        Z = []

        for landmark in landmarks:
            dy = (self.y - landmark[0])
            dx = (self.x - landmark[1])
            Z.append(pi + atan2(dy, dx) - self.orientation)
            if noisy:
                Z[-1] += random.gauss(0.0, self.bearing_noise)
            #Z.append(sqrt(dy**2 + dx**2))
    ...
    def move(self, motion):  # Do not change the name of this function
        ...
        if alpha > max_steering_angle:
            raise ValueError('Exceeding max steering wheel: pi/4')
        if d < 0:
            raise ValueError('Impossible moving backwards')

        # apply noise
        alpha2 = random.gauss(alpha, self.steering_noise)
        d2 = random.gauss(d, self.distance_noise)

        # compute turn and orientation
        beta = d2 / self.length * tan(alpha2)
        neworient = (self.orientation + beta)
        neworient %= (2 * pi)
        # if the turn is significant
        if beta > tol:
            R = d2/beta
            Cx = self.x - R * sin(self.orientation)
            Cy = self.y + R * cos(self.orientation)
            newx = Cx + R * sin(self.orientation + beta)
            newy = Cy - R * cos(self.orientation + beta)

        else:
            # otherwise go forward
            newx = self.x + d2 * cos(self.orientation)
            newy = self.y - d2 * sin(self.orientation)

        # update values
        result.set(newx, newy, neworient)
        return result
