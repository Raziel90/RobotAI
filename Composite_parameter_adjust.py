# Parameter adjustment with Twiddle Algorithm
...
p_gain = 2.0
d_gain = 6.0
weight_data, weight_smooth, p_gain, d_gain = [-2.1351495099999998, 0.87561, 0.23189000000000037, 2.188110000000001]
###################################

err_fun = lambda d_w, s_w, p_g, d_g: sum(
    [main(grid, init, goal, steering_noise, distance_noise, measurement_noise,
          d_w, s_w, p_g, d_g) for k in range(5)][:][1])


def twiddle(err_fun, init_parameters, tol=0.01):
    p = [0] * len(init_parameters)
    dp = [1] * len(init_parameters)
    best_error = err_fun(*init_parameters)
    best_par = init_parameters
    while best_error > tol:
        try:
            ...
            print(e)
    best_par = p
    return best_par, best_error


pars, err = twiddle(err_fun, (weight_data, weight_smooth, p_gain, d_gain))
